...
const ProjectsSection: React.FC = () => {
    ...
    return (
        <div className='px-2'>
            ...
            <section className='grid grid-cols-4 md:grid-cols-5 gap-2 px-3 md:px-2 mt-2 mb-6'>
...
const ProjectCard: React.FC<{ url: string; title: string; id: string }> = ({
    ...
    return (
        <Link
            ...
            to={`/projects/${id}`}
            className='_project-card col-span-2 md:col-span-1 no-underline decoration-current text-current rounded'
        >
            <div className='_overlay rounded'>
                <span>
                    <h1 className='text-xl font-thin text-center text-white'>
                        {title}
                    </h1>
                </span>
            </div>
